...
<body class="header-light sidebar-dark sidebar-expand">
                        ...
                        <div class="col-md-12 widget-holder">
                            <div class="widget-bg">
                                ...
                                <div class="widget-body clearfix">
                                    ...
                                    <div class="card mt-3" id="deviceInfoCard" style="display: none;">
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            $("#checkDevice").on("submit", function(e){
                e.preventDefault();
                $("#deviceInfoCard").hide();
                $("#deviceInfoCard").find("input").val();
                $("#activateDevice").prop("disabled", false);

                // $("#deviceInfoCard").children("form").reset();
                ...
                $.ajax({
                    ...
                    success: function(res){
                        if(res.success){
                            ...
                            swal("Error", res.error || "An error occurred.", "error");
                            token = res._token;
                        }
                    },
                    error: function(){
            ...
            $("#activateDevice").on("click", function(e){
                if(deviceInfo.activated){
                    swal("Error", "Device is already activated.", "error");
                    return;
                }
                if(!deviceInfo || !deviceInfo.device_id){
                    swal("Error", "Please check device info first.", "error");
                    return;
                }
                $("#activationModal").modal("show");
            });

            $("#activateDeviceBtn").on("click", function(e){
                var duration = $("#activation-duration").val();
                if(!duration || (duration != "0" && duration != "1" && duration != "2")){
                    swal("Error", "Please select a valid duration.", "error");
                    return;
                }
                $.ajax({
                    url: "/activation/device/activate",
                    method: "POST",
                    data: {
                    ...
                    success: function(res){
                        if(res.success){
                            token = res._token;
                            swal("Success", "Device activated successfully.", "success");
